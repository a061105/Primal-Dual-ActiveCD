...
int main(int argc, char** argv){
	...
	if( argc < 1+7 ){
		cerr << "./parallelRCD [data] [testdata] [L1_lambda] [L2_lambda] [loss(0:square,1:L2-hinge,2:logistic,3:SmoothHinge)] [num_threads] [num_iter] (stop_obj)" << endl;
		exit(0);
	}
	...
	double lambda2 = atof(argv[4]);
	int loss_to_use = atoi(argv[5]);
	int nThreads =atoi(argv[6]);
	int nIter = atoi(argv[7]);
	
	double stop_obj = -1e300;
	if( argc > 1+7 ){
		stop_obj = atof(argv[8]);
	}
	...
	else if( loss_to_use==2 )
		loss = new LogisticLoss();
	else
		loss = new SmoothHingeLoss();
			
	double* w = new double[d];
	rcd(features,labels,n,loss,lambda,lambda2,w,nThreads, nIter, stop_obj);
