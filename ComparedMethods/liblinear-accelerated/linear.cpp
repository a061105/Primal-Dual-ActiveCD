	...
	double Cp, double Cn, int solver_type)
{
	...
	int *index = new int[l];
	double *alpha = new double[l]; //alpha_{t+1} = rho^{t+1}u_{t+1} + v_{t+1}
	schar *y = new schar[l];
	int active_size = l;
	...
	//acceleration maintains vector u, v ( y = rho^{t+1}*u_t + v_t )
	double* u = new double[l];
	double* z = new double[l];
	for(i=0;i<l;i++){
		u[i] = 0.0;
		z[i] = 0.0; //alpha=0 initially
	}
	//aceeleration maintains w = A'y = rho^{t+1}A'u_t + A'v_t = rho^{t+1} w_u + w_v
	double* w_u = new double[w_size];
	double* w_z = new double[w_size];
	for(i=0;i<w_size;i++){
		w_u[i] = 0.0;
		w_z[i] = 0.0; //alpha=0 initially
	}
	
	double h, zi, theta_sq, theta_l, theta_old=1.0/l,  theta_sq_old=theta_old*theta_old;
	while (iter < max_iter)
	{
		PGmax_new = -INF;
		PGmin_new = INF;
		...
		for (s=0; s<active_size; s++)
		{
			...
			while(xi->index!= -1)
			{
				G += (theta_sq*w_u[xi->index-1] + w_z[xi->index-1]) * (xi->value);
				xi++;
			}
			G = G*yi-1;
			
			C = upper_bound[GETI(i)];
			G += (theta_sq*u[i]+z[i])*diag[GETI(i)]; //evaluate gradient at y
			...
			if(fabs(PG) > 1.0e-12)
			{
				h = min(max( zi - G/(theta_l*QD[i]), 0.0), C) - zi;
				double delta_u = -(1-theta_l)*h/theta_sq;
				double delta_z = h;
				...
				while (xi->index != -1)
				{
					w_u[xi->index-1] += delta_u*xi->value;
					w_z[xi->index-1] += delta_z*xi->value;
					xi++;
				}
			}
			
			theta_old = theta;
			theta_sq_old = theta_sq;
			
			theta = (sqrt( theta_sq*theta_sq + 4*theta_sq ) - theta_sq)/2;
		}
	...
	// calculate alpha from u, v
	for(i=0;i<l;i++)
		alpha[i] = z[i];
		//alpha[i] = theta_old_sq*u[i] + z[i];
	
	// calculate w from w_u, w_v
	for(i=0;i<w_size;i++){
		w[i] = 0;
	}
	for(i=0;i<l;i++){
		double ya = y[i] * alpha[i];
		feature_node *xi = prob->x[i];
		while (xi->index != -1){
			w[xi->index-1] += ya*xi->value;
			xi++;
		}
	}
	...
	for(i=0; i<l; i++)
	{
		fval += alpha[i]*(alpha[i]*diag[GETI(i)] - 2);
		if( alpha[i] > 1e-12 )
			++nSV;
	}
	info("Objective value = %lf\n",fval/2);
	...
	double Cp, double Cn)
{
	...
	while(newton_iter < max_newton_iter)
	{
		...
		info("iter %3d, time=%f,  #CD cycles %d\n", newton_iter, end-start, iter);
	}
