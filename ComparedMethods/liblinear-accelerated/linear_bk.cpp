	...
	double Cp, double Cn, int solver_type)
{
	...
	if( solver_type == L2R_L1LOSS_SVC_DUAL ){
		...
		mu = 1.0 / (1.0 + 2*Cp*R_sq);
	}
	double a = sqrt(mu)/l;
	double rho = (1-a) / (1+a), rho_t = rho;
	...
	while (iter < max_iter)
	{
		...
		for (s=0; s<active_size; s++)
		{
			...
			while(xi->index!= -1)
			{
				G += (rho_t*w_u[xi->index-1] + w_v[xi->index-1]) * (xi->value);
				xi++;
			}
			G = G*yi-1;
			
			C = upper_bound[GETI(i)];
			G += (rho_t*u[i]+v[i])*diag[GETI(i)]; //evaluate gradient at y
			
			z_y_intp = -rho_t*u[i]+v[i]; //z_y_intp = (1-a)z + ay
			PG = 0;
			if ( fabs(z_y_intp) < 1e-12 )
			{
				if (G > PGmax_old)
				{
					//active_size--;
			...
			if(fabs(PG) > 1.0e-12)
			{
				...
				h = min(max( z_y_intp - G/(sqrt_mu*QD[i]), 0.0), C) - z_y_intp;
				double delta_u = -(1-sqrt_mu)*h/(2*rho_t);
				double delta_v = (1+sqrt_mu)*h/2;
				...
				while (xi->index != -1)
				{
					...
					w_v[xi->index-1] += delta_v*xi->value;
					xi++;
				}
			}
