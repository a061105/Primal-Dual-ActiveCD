...
int main(int argc, char** argv){
	...
	for(int iter=0; iter<max_iter; iter++){
		...
		search_a_active( w, w_act_index, Xt, alpha,    a_ind_new, num_a_select, N );
		sa_time += omp_get_wtime();
		
		sw_time -= omp_get_wtime();
		search_w_active( alpha, a_act_index, w_is_act, X, lambda, w,    w_ind_new, num_w_select, D );
		sw_time += omp_get_wtime();
		 ...
		 */
		int a_act_size = a_act_index.size();
		int w_act_size = w_act_index.size();
		int max_inner = (int)(0.05*max( (double)N/a_act_size, (double)D/w_act_size ));
		max_inner = min( max(max_inner,1), 100 );
		//int max_inner = 10;
		...
		for(deque<int>::iterator it=a_act_index.begin(); it!=a_act_index.end(); it++)
			if( alpha[*it]  != 0.0 )
				tmp.push_back(*it);
		a_act_index = tmp;
		num_a_select = a_act_index.size();
		num_a_select = max( num_a_select, 10 );
		...
		w_act_index = tmp;
		*/
		num_w_select = w_act_index.size();
		num_w_select = max(num_w_select, 10);
